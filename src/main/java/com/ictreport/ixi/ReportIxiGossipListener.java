...
public class ReportIxiGossipListener implements GossipListener {
    ...
    private final PingPayloadProcessor pingPayloadProcessor = new PingPayloadProcessor();
    ...
    public void onGossipEvent(final GossipEvent event) {
        ...
        if (!event.isOwnTransaction()) {
            if (reportIxi != null) {
                try {
                    final Payload payload = Payload.deserialize(event.getTransaction().decodedSignatureFragments());
                    pingPayloadProcessor.process(reportIxi, payload);
